...
Page({
  ...
  setNumber: function (e) {
    ...
    if (cartList[index].productInfo.attrInfo && cartList[index].productInfo.attrInfo.stock){
      if (cart_num > cartList[index].productInfo.attrInfo.stock){
            wx.showToast({
              title: '库存不足',
              icon: 'none',
              duration: 2000
            });
             cart_num = cartList[index].productInfo.attrInfo.stock;
      }
    }else{

      if (cartList[index].productInfo.stock) {
        if (cart_num > cartList[index].productInfo.stock) {
          wx.showToast({
            title: '库存不足',
            icon: 'none',
            duration: 2000
          });
          cart_num = cartList[index].productInfo.stock;
        }
      } 
  ...
  numAddClick: function (event) {
    ...
    var cartList = this.data.cartList;
        cartList[index].cart_num = +cartList[index].cart_num + 1;

    if (cartList[index].productInfo.attrInfo && cartList[index].productInfo.attrInfo.stock) {
      if (cartList[index].cart_num > cartList[index].productInfo.attrInfo.stock) {
        wx.showToast({
          title: '库存不足',
          icon: 'none',
          duration: 2000
        });
        cartList[index].cart_num = cartList[index].productInfo.attrInfo.stock;
      }
    } else {

      if (cartList[index].productInfo.stock) {
        if (cartList[index].cart_num > cartList[index].productInfo.stock) {
          wx.showToast({
            title: '库存不足',
            icon: 'none',
            duration: 2000
          });
          cartList[index].cart_num = cartList[index].productInfo.stock;
        }
      }
    ...
    var minusStatus = cartList[index].cart_num <= 1 ? 'disabled' : 'normal';
    this.setData({ cartList: cartList, minusStatus: minusStatus});
    this.carnum();
    this.countmoney();
    this.addCartNum(cartList[index].cart_num,cartList[index].id);
  },
  //减
