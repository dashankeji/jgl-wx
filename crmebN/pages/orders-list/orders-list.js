  ...
  getorderlist:function(e){
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        for (var i = 0; i < len; i++) {
          for (var key in $data[i].cartInfo) {
            var date = new Date($data[i].cartInfo[key].add_time * 1000);
            $data[i].add_time = date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate() + ' ' + date.getHours() + ':' + date.getMinutes() + ':' + date.getSeconds();
          }
